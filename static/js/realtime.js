...
var Ami = {
    ...
    _addCall: function(acall, recording) {
        ...
        var actions = '';
        if (recording) actions += '<button class="btn btn-warning disabled" disabled><i class="zmdi zmdi-rotate-right zmdi-hc-spin zmdi-hc-lg"></i> In registrazione</button>&nbsp;';
        else actions += '<button class="btn btn-warning record-call" onclick="Ami.recordCall(\'' + acall.channel + '\',\'' + filename + '\')">Registra</button>&nbsp;';
        actions += '<button class="btn btn-danger hangup-call" onclick="Ami.hangUpCall(\'' + acall.channel + '\')">Riaggancia</button>';
